...
static void (*color_avg_intern)(const struct Color, struct Color, uint8_t);

static void (*avg_table[INST_SETS][SPACES][BOOLS])(const struct Color,
    ...
    [FB]  = {
	 [OK]  = {[true] = oklab_avg_fallback_cw, [false] = oklab_avg_fallback},
	 [CIE] = {
	     [true] = cielab_avg_fallback_cw, [false] = cielab_avg_fallback}}};
...
void color_avg(const struct Color colors, struct Color cents, uint8_t which_cent) {
	check_initialized();
