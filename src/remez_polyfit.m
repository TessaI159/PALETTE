...
fprintf("\nC-style %s coefficient array (%d terms):\n", mode,
	length(filtered_coeffs));
fprintf("\n/*\n*Minimax polynomial approximation on [%f, %f].\n*Degree: %d\n*Max\
	absolute error: %.17f\n*Generated by remez_polyfit.m in GNU Octave\n*/\n",
	-a, a, n, err);
fprintf("float %s[%d] = {", array_name, length(filtered_coeffs));
for i = 1:length(filtered_coeffs)
  if i < length(filtered_coeffs)
    fprintf("%.8ff, ", filtered_coeffs(i));
  else
    fprintf("%.8ff", filtered_coeffs(i));
  end
end
...
for i = 2:length(rev_coeffs)
  ...
  if gap == 1
    fprintf("    y = y * x + coeffs[%d];\n", i - 1);
  else
    fprintf("    y = y * x * x + coeffs[%d];\n", i - 1);
  end
end
fprintf("    return y * x;\n");
end
